...
RSpec.describe ComposeFile do
  ...
  describe '.load' do
    it 'loads an existing file' do
      ...
      expect(compose.services).to be_a(Hash)
      expect(compose.services.keys).to include('helios', 'postgresql', 'redis')
    end

    it 'handles non-existent file' do
      compose = described_class.load('/nonexistent/compose.yaml')
      expect(compose.services).to eq({})
    end
  ...
  describe '#services' do
    ...
    it 'returns all services' do
      expect(compose.services.keys).to eq(%w[helios postgresql redis])
    end
  ...
  describe '#add_service' do
    ...
    it 'adds a new service with hash config' do
      ...
      expect(compose.service?('watchtower')).to be true
      expect(compose.service('watchtower')['image']).to eq(
    ...
    it 'adds a service with nested configuration' do
      ...
      service = compose.service('influxdb')
    ...
    it 'accepts symbol keys and converts them to strings' do
      ...
      expect(compose.service?('newservice')).to be true
    end
  end
  ...
  describe '#remove_service' do
    ...
    it 'removes an existing service' do
      ...
      expect(compose.service?('redis')).to be false
    end
  ...
  describe '#service' do
    ...
    it 'returns the service configuration' do
      service = compose.service('postgresql')
      expect(service['image']).to eq('postgres:18-alpine')
    end

    it 'returns nil for non-existent service' do
      expect(compose.service('nonexistent')).to be_nil
    end
  end
  ...
  describe '#service?' do
    ...
    it 'returns true for existing service' do
      expect(compose.service?('helios')).to be true
    end

    it 'returns false for non-existent service' do
      expect(compose.service?('nonexistent')).to be false
    end
  end
  ...
  describe '#save' do
    ...
    it 'preserves existing services when modifying' do
      ...
      expect(reloaded.services.keys).to include(
        'helios',
        'postgresql',
        'redis',
        'watchtower',
