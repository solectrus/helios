RSpec.describe Surveys::Shelly::Survey do
  describe '#call' do
    ...
    context 'when the registry exposes multiple versions (default SHELLY_COLLECTOR)' do
      ...
      it 'fills the image element choices with the current image first' do
        expect(image_element['choices'].first).to include(
          'value' => DockerImages.current(:SHELLY_COLLECTOR),
        )
      end
      ...
      it 'includes every selectable variant from the registry' do
        expect(image_element['choices'].pluck('value'))
          .to eq(DockerImages::SHELLY_COLLECTOR[:current].pluck(:image))
      end

      it 'preselects the recommended (first) variant so fresh setups default to latest' do
        expect(image_element['defaultValue']).to eq(DockerImages.current(:SHELLY_COLLECTOR))
      end
    end
    ...
    context 'when the registry only exposes a single version' do
      before do
        stub_const(
          'DockerImages::SHELLY_COLLECTOR',
          { current: 'ghcr.io/solectrus/shelly-collector:latest' }.freeze,
        )
      end
