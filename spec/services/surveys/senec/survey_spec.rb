RSpec.describe Surveys::Senec::Survey do
  describe '#call' do
    ...
    context 'when the registry exposes multiple versions (default SENEC_COLLECTOR)' do
      ...
      it 'fills the image element choices with the current image first' do
        expect(image_element['choices'].first).to include(
          'value' => DockerImages.current(:SENEC_COLLECTOR),
        )
      end
      ...
      it 'includes every selectable variant from the registry' do
        expect(image_element['choices'].pluck('value'))
          .to eq(DockerImages::SENEC_COLLECTOR[:current].pluck(:image))
      end

      it 'preselects the recommended (first) variant so fresh setups default to latest' do
        expect(image_element['defaultValue']).to eq(DockerImages.current(:SENEC_COLLECTOR))
      end
    end
    ...
    context 'when the registry only exposes a single version' do
      before do
        stub_const(
          'DockerImages::SENEC_COLLECTOR',
          { current: 'ghcr.io/solectrus/senec-collector:latest' }.freeze,
        )
      end
