...
- **`POWER_SPLITTER_INTERVAL=300` emitted (donor had it commented).**
  ...
  commented; at runtime the collector fell back to its built-in
  default (`3600`). HELIOS pins the var to a fixed `300` (5-minute
  cadence) for every stack. Same as user13/14.
