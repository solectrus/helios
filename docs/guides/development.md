...
SKIP_INTEGRATION=1 bin/ci                      # full CI without the integration specs (~1 min)
...
- `spec/scenarios/` — whole configurations recorded end to end, each with the input HELIOS starts from and the snapshot it must produce: `setup/` (the answers of a new installation), `import/` (a foreign stack), `export/` (a `config.yaml` of an older release). Record the snapshots anew with `UPDATE_SNAPSHOTS=1 bin/rspec spec/scenarios`; see `spec/scenarios/README.md`
