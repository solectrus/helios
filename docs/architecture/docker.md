...
| `mosquitto`          | When the MQTT survey asks HELIOS to run the broker                         |
| `ingest`             | When a sensor is flagged as a balcony power plant and the correction is on |
...
Mosquitto 2.x listens on localhost only until a configuration file names a listener, and it reads no listener or password option from the command line. HELIOS writes two files outside its own data directory, `compose.yaml` and `.env`, so the broker builds its configuration itself. The `command` of the service writes the configuration file, writes a password file when credentials are set, and then replaces itself with the broker process.

Three details of the image drive that command:

1. The container starts as root, and the broker drops to the user `mosquitto` (UID 1883). A password file written by the command must change hands, or the broker cannot read it.
2. `mosquitto_passwd -c` refuses to overwrite an existing file. A restarted container still carries the file from its last start, so the command deletes it first.
3. The file `/mosquitto-no-auth.conf` in the image opens an extra HTTP listener and switches persistence off. HELIOS writes its own file in both cases, so retained messages survive a restart.

The health check reads the listening socket with `netstat` instead of subscribing to a topic. A subscribing client logs a connection on every run, which hides the connections of the real devices in the log. The broker refuses to start when its configuration or its password file is unusable, so a bound socket says the same thing.

The credentials travel through `.env` as `MQTT_USERNAME` and `MQTT_PASSWORD`. They never appear in `compose.yaml`. The collector reads the same two variables and reaches the broker at `mosquitto:1883` inside the compose network. Only the published host port is configurable, because the host can already run a broker of its own.

#### How a device reaches the broker

Behind the Traefik that HELIOS runs, the broker gets two entrypoints. `mqtt` carries plain MQTT on the published port, and `mqtts` carries the encrypted one on port 8883. The broker then publishes no port of its own, and Traefik routes both entrypoints to it (see [`Export::Services::Mosquitto`](../../app/services/export/services/mosquitto.rb)).

In every other case the broker publishes its host port, and a device speaks plain MQTT on it. This holds without a proxy, and behind an external proxy as well. `Export::Compose::EXTERNALLY_ROUTABLE` names the four HTTP services alone, so no external proxy routes the TCP stream of the broker. A device that must reach the broker over TLS needs the Traefik that HELIOS runs.

#### The owner of the persistence file

The broker can log this warning at start:

```
Warning: File /mosquitto/data//mosquitto.db owner is not mosquitto.
Future versions will refuse to load this file.
```

The entrypoint of the image runs as root and does a `chown -R 1883:1883 /mosquitto/data` on every start, so the warning does not appear on a normal Linux host. It appears on a file system that ignores ownership and answers `chown` with success without a change. Two cases are known: a bind mount under Docker Desktop on macOS, which is a development machine, and a storage location on a share that maps every file to one user, for example Unraid under `/mnt/user` or an SMB mount.

HELIOS cannot repair that. A `chown` in the `command` of the service fails in exactly the same places as the one of the image. Today the broker loads the file and only warns. If a later version of Mosquitto keeps its announcement, the broker discards the file on such a host and loses the retained messages at every restart. The fix then belongs to the host: put the storage location of the broker on a file system that keeps ownership.

### Networks

Every service joins the network of the stack, named `solectrus_default` by default. A service reaches every other service on it by service name.

An external reverse proxy reaches the stack in one of two ways. The address form stores which one (see [`Configuration#reverse_proxy_network`](../../app/models/configuration.rb)):

- Published host ports. The proxy runs on the host, or on another machine, and routes to `<bind_ip>:<port>`. HELIOS generates a Traefik file-provider snippet for it, which the user copies into that proxy.
- A shared Docker network. The proxy runs in a stack of its own and joins the same network. The routed services (dashboard, InfluxDB, Ingest, HELIOS) then join that network as well, publish no host port at all, and the proxy reaches each one by service name. When the configuration names a Traefik entrypoint, HELIOS also writes the router labels for those services, so the routes appear without any file to copy.

HELIOS declares the second network as `external: true`, because the stack of the proxy owns it. Compose refuses to start when the network is missing.

A stack adopted from a parent compose stack can carry such a network without a proxy in play. The network is then stored the same way, every service joins it again, and the published ports stay.
...
| Mosquitto           | `eclipse-mosquitto:2.1-alpine`                | Minor version pinned          |
