module DockerHost
  class StackStatus
    ...
    def refresh!
      ...
      @initialized.make_true
      Rails.logger.error(
        "DockerHost::StackStatus.refresh! failed: #{e.class}: #{e.message}",
      )
    end
    ...
    def refresh_service_statuses
      ...
      containers_by_name = DockerHost::Container.all.index_by(&:service_name)
    ...
    def broadcast!
      StatusBar::Broadcaster.new.broadcast
    rescue StandardError => e
      Rails.logger.error(
        "DockerHost::StackStatus broadcast failed: #{e.class}: #{e.message}",
      )
    end
  end
end
